...
def killme(api_instance):
  return api_instance.api_problems_topic_get(topic="all")
...
_ = [problems.setdefault(p.get('difficulty', {}).get('level', ''), {}).setdefault(p.get('stat', {}).get('question__title_slug'), 
        '/'.join([LEET_CODE_URL, 'problems', p.get('stat', {}).get('question__title_slug')])), .setdefault(p.get('stat', {}).get('question__title_slug'), 
        '/'.join([LEET_CODE_URL, 'problems', p.get('stat', {}).get('question_id')]))
